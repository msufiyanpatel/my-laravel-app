	...
	try {
		foreach ($allData as $key => $value) {
			...
			if ($key == 'url') {
				...
				writeConfig("site_url", $value);
			}
			if ($key == 'app_name') {
				writeEnv("APP_NAME", $value);
				writeEnv("APP_ENV", "production");
				writeConfig("site_name", $value);
			}
...
function sendAcknowledgement($val)
{
...
<head>
	...
	<title>Easy Installer by PixWin</title>
...
<body>
	...
	<div class="installation-section padding-bottom padding-top">
		<div class="container">
									...
									if ($_POST) {
										...
										if (!importDatabase($alldata)) {
											...
											if (!envSetup($alldata)) {
												...
												<a href="' . getWebURL() . '/login" class="theme-button choto">Go to Admin Panel</a>
												</div>';
											}
										}
									}
			...
			?>
				<div class="installation-wrapper pt-md-4">
					<ul class="installation-menu ">
						<li class="steps done">
							<div class="thumb">
								<span>
							...
							<div class="box-item">
								<form action="?action=complete" method="post" class="information-form-area mb--20">
									<div class="info-item">
										...
										<div class="row">
											<div class="information-form-group col-md-6">
							...
							<div class="box-item">
								<div class="item table-area">
									<table class="requirment-table">
										...
										$envFileAccess = file_exists('../../.env');
										if ($envFileAccess == true) {
											$error = $error + 0;
											tableRow('.env', '  Required ".env" available', 1);
										} else {
											$error = $error + 1;
											tableRow('.env', ' Required ".env" available', 0);
										}
										?>
									</table>
								</div>
	...
	<footer class="section-bg py-3 text-center">
		<div class="container">
			<p class="m-0 footer-text">&copy;<?php echo Date('Y') ?> - All Right Reserved by PixWin</p>
		</div>
	</footer>
	<style>
		#hide {
